 ...
 */
@WebServlet(name = "addProfilePic", urlPatterns = {"/addProfilePic"})
public class addProfilePic extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String image = request.getParameter("pro_pic");
        ...
        if (user_id != null && image != null) {
            ...
            user.setPropicture(image.getBytes());
            hib.session.save(user);
            hib.transaction.commit();
            request.getSession().setAttribute("loguser", user);
            response.sendRedirect("member.jsp?msg=Done");
        }
        else{
            response.sendRedirect("member.jsp?msg=Error '"+user_id+"'and '"+image+"'");
        }
    }
